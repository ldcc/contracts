...
contract chromosphere {
    ...
    event Withdraw(address payable indexed _drawer, address indexed _to, address indexed _currency, uint256 _value);
    ...
    function answerOf() external view returns (uint8[7] memory balls) {
        balls = answer;
    }
    ...
    function enter(uint8[] memory _reds, uint8[] memory _blues) public payable {
        require(active);
        require(now < deadline);
        ...
        if (pool == address(0)) {
            ...
            bytes memory payload = abi.encodeWithSignature("transferFrom(address,address,uint256)", msg.sender, address(this), stake);
            (bool success,) = pool.call.gas(90000)(payload);
            require(success);
        }
    ...
    function _takePrize(address _winner, uint256 _bonus) private {
        if (pool == address(0)) {
            ...
            address(uint160(_winner)).transfer(_bonus);
        } else {
            bytes memory payload = abi.encodeWithSignature("transfer(address,uint256)", _winner, _bonus);
            (bool success,) = pool.call.gas(gasleft())(payload);
            require(success);
        }
    }

    function _evalvel(Gambler memory _gambler) private view returns (uint8 level, uint256 bonus) {
    ...
    function _randBall(bytes32 _hash, uint8 _r) private view returns (uint8) {
